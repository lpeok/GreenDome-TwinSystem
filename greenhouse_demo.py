...
# --- 侧边栏：模拟环境输入 ---
st.sidebar.header("感知层数据模拟面板")
st.sidebar.markdown("拖动滑块模拟传感器实时数据与气象预报：")

T_in = st.sidebar.slider("室内实时温度 (℃)", 15.0, 45.0, 33.0, step=0.5)
T_out = st.sidebar.slider("未来1h室外预测温度 (℃)", 10.0, 40.0, 22.0, step=0.5)
...
if T_in > T_set:
    ...
    if T_out < (T_in - delta):
        cooling_strategy = "主动节能模式：气象对冲被动降温"
        ac_status = "强制封锁 (关闭)"
